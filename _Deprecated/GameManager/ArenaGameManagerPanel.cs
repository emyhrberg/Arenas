...
//        countdown = CreateTimingSlider("Freeze", 300, DefaultCountdownSeconds(), value => $"{value:0}s", GamePhase.FreezeCountdown, ArenaGameManagerNetHandler.ActionType.SetCountdown);
//        roundTime = CreateTimingSlider("Round", 3600, selected?.RoundDurationSeconds ?? 600, value => ArenaGameManagerText.Time((int)value), GamePhase.Playing, ArenaGameManagerNetHandler.ActionType.SetRoundTime, 1);
//        votingTime = CreateTimingSlider("Vote", 300, Config.VotingDurationSeconds, value => $"{value:0}s", GamePhase.Voting, ArenaGameManagerNetHandler.ActionType.SetVotingTime);
...
//        if (!countdown.IsHeld && ArenaRoundSystem.Phase == GamePhase.FreezeCountdown)
//            countdown.SetValue(SecondsLeft());
//        if (!roundTime.IsHeld && ArenaRoundSystem.Phase == GamePhase.Playing)
//            roundTime.SetValue(SecondsLeft());
//        if (!votingTime.IsHeld && ArenaRoundSystem.Phase == GamePhase.Voting)
...
//    private UILabeledSlider CreateTimingSlider(string label, int max, int value, Func<float, string> format, GamePhase phase, ArenaGameManagerNetHandler.ActionType action, int min = 0)
//    {
...
//        section.Add(new ArenaManagerButton(() => ArenaRoundSystem.Phase is GamePhase.FreezeCountdown or GamePhase.Playing ? "Restart Fight" : "Start Fight", Ass.IconStartGame, StartFight, CanStartFight, StartFightReason), 76);
...
//        section.Add(new ArenaManagerButton(AdvanceText, Ass.IconRefresh, () => Request(ArenaGameManagerNetHandler.ActionType.AdvancePhase), () => InArena() && ArenaRoundSystem.Phase is GamePhase.FreezeCountdown or GamePhase.Voting, AdvanceTooltip), 190);
//        section.Add(new ArenaManagerButton(() => ArenaRoundSystem.Phase == GamePhase.Generating ? "Changing Arena" : "End Round", Ass.IconEndGame, () => Request(ArenaGameManagerNetHandler.ActionType.EndRound), () => InArena() && ArenaRoundSystem.Phase is GamePhase.Generating or GamePhase.FreezeCountdown or GamePhase.Playing, () => ArenaRoundSystem.Phase == GamePhase.Generating ? "The next arena is loading" : "End the round and open voting", true), 228);
//    }
...
//        && ArenaRoundSystem.Phase is GamePhase.Ready or GamePhase.FreezeCountdown or GamePhase.Playing;
//    private static bool CanBalance() => (!SubworldSystem.AnyActive() || InArena())
//        && ArenaRoundSystem.Phase is GamePhase.Idle or GamePhase.Ready
...
//    private static bool FreezeOrPlaying(GamePhase phase) => phase is GamePhase.FreezeCountdown or GamePhase.Playing;
//    private static string AdvanceText() => ArenaRoundSystem.Phase == GamePhase.FreezeCountdown ? "Start Fight" : ArenaRoundSystem.Phase == GamePhase.Voting ? "End Voting" : "Next Phase";
//    private static string AdvanceTooltip() => ArenaRoundSystem.Phase == GamePhase.FreezeCountdown ? "Skip the countdown" : "End voting now";

//    private void RefreshValues()
//    {
//        countdown.SetValue(ArenaRoundSystem.Phase == GamePhase.FreezeCountdown ? SecondsLeft() : DefaultCountdownSeconds());
//        roundTime.SetValue(ArenaRoundSystem.Phase == GamePhase.Playing ? SecondsLeft() : SelectedPreset?.RoundDurationSeconds ?? 600);
//        votingTime.SetValue(ArenaRoundSystem.Phase == GamePhase.Voting ? SecondsLeft() : Config.VotingDurationSeconds);
//    }
...
//            return;
//        if (ArenaRoundSystem.Phase != GamePhase.FreezeCountdown)
//            countdown.SetValue(DefaultCountdownSeconds());
//        if (ArenaRoundSystem.Phase != GamePhase.Playing)
//            roundTime.SetValue(SelectedPreset.RoundDurationSeconds);
//        if (ArenaRoundSystem.Phase != GamePhase.Voting)
