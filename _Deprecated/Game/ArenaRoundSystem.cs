...
//using Terraria.ID;
//using Arenas.Common.Game.EndScreen;

//namespace Arenas.Common.Rounds;

//internal enum GamePhase : byte { Idle, Generating, Ready, Sandbox, FreezeCountdown, Playing, Voting }
...
//    private static bool winningAttributionExact;
...
//    public static IReadOnlyList<RoundPlayerStats> Scoreboard => Main.netMode == NetmodeID.MultiplayerClient || Phase == GamePhase.Voting ? scoreboard : LiveStats();
...
//        {
//            if (!ArenaWorldSystem.MatchReady || Phase is GamePhase.Generating or GamePhase.FreezeCountdown)
//                Main.LocalPlayer.AddBuff(BuffID.Frozen, 2);
//            if (!IsTimerPaused && Phase != GamePhase.Idle && RemainingTicks > 0) RemainingTicks--;
//            return;
//        }

//        if (!ArenaWorldSystem.MatchReady && Phase != GamePhase.Generating)
//        {
//            HoldPlayersInLobby();
//            return;
//        }
...
//        switch (Phase)
//        {
//            case GamePhase.Idle: break;
//            case GamePhase.Generating: TickGenerating(); break;
//            case GamePhase.Ready: break;
//            case GamePhase.Sandbox: break;
//            case GamePhase.FreezeCountdown: TickFreeze(); break;
//            case GamePhase.Playing: TickPlaying(); break;
//            case GamePhase.Voting: TickVoting(); break;
//        }

//        if (Main.netMode == NetmodeID.Server && Main.GameUpdateCount % 60 == 0) RoundNetHandler.SendStateToAll();
//    }
...
//    public static bool IsSandboxActive => ArenaWorldSystem.Active && Phase == GamePhase.Sandbox
...
//    public static bool RequiresClassSelection => Phase is GamePhase.FreezeCountdown or GamePhase.Playing
...
//    {
//        if (player == null || string.IsNullOrEmpty(characterKey) || Phase == GamePhase.Idle)
//            return false;
...
//            return Phase is GamePhase.FreezeCountdown or GamePhase.Playing
//                ? ActivateLateParticipant(candidate, player)
//                : Phase is GamePhase.Generating or GamePhase.Ready;
//        }
...
//    {
//        if (Main.netMode == NetmodeID.MultiplayerClient) RoundNetHandler.SendVote(index);
...
//    {
//        if (Main.netMode == NetmodeID.MultiplayerClient)
//            RoundNetHandler.SendClassRequest(arenaClass);
//        else
...
//            RoundNetHandler.SendClassState(playerId, arenaClass, CurrentPresetIndex);
//    }
...
//    {
//        if (Phase != GamePhase.Voting || !IsParticipant(playerId) || index < 0 || index >= GetValidPresets().Count) return;
...
//    internal static void ApplyState(GamePhase phase, RoundResult result, Team winningTeam, int winningPlayerId, int ticks, int preset, int localVote, bool paused, int nextGenerationId, float progress, ArenaLayout layout, List<int> counts, List<List<byte>> voters, List<RoundPlayerStats> entries)
//    {
//        Phase = phase; Result = result; RemainingTicks = ticks; CurrentPresetIndex = preset; LocalVote = localVote;
//        WinningTeam = winningTeam;
//        WinningPlayerId = winningPlayerId;
//        if (Main.netMode == NetmodeID.MultiplayerClient && phase != GamePhase.Playing)
//        {
//            bossIndex = -1;
//            bossType = 0;
//        }
...
//        else if (phase == GamePhase.Generating) ArenaWorldSystem.ApplyNetworkLayout(null);
...
//    {
//        if (Main.netMode == NetmodeID.MultiplayerClient || !ArenaWorldSystem.Active || !ArenaWorldSystem.MatchReady
//            || Phase is not (GamePhase.Ready or GamePhase.FreezeCountdown or GamePhase.Playing)
...
//            || Phase is not (GamePhase.Idle or GamePhase.Ready)) return;
...
//    internal static void AdminSetCountdown(int seconds) { if (Phase == GamePhase.FreezeCountdown) SetRemaining(seconds, 300); }
//    internal static void AdminSetRoundTime(int seconds) { if (Phase == GamePhase.Playing) SetRemaining(seconds, 3600); }
//    internal static void AdminSetVotingTime(int seconds) { if (Phase == GamePhase.Voting) SetRemaining(seconds, 300); }

//    internal static void AdminTogglePause()
//    {
//        if (Phase is GamePhase.Idle or GamePhase.Generating or GamePhase.Ready) return;
//        IsTimerPaused = !IsTimerPaused; RoundNetHandler.SendStateToAll();
//    }

//    internal static void AdminAdvancePhase()
//    {
//        if (Phase == GamePhase.FreezeCountdown) StartPlaying();
//        else if (Phase == GamePhase.Voting) ResolveVoting();
//    }
...
//        }
//        else if (Phase is GamePhase.FreezeCountdown or GamePhase.Playing) EndRound(RoundResult.AdminEnded);
//    }

//    internal static void AbortForWorldRegeneration()
//    {
//        if (Main.netMode == NetmodeID.MultiplayerClient)
//            return;
//        if (Phase != GamePhase.Idle)
//            Log.Info($"Aborting round state {Phase} because the Arenas world is being regenerated.");
//        SetIdle(RoundResult.AdminEnded);
//    }

//    internal static void MarkWorldGenerationFailed(string reason)
//    {
//        if (Main.netMode == NetmodeID.MultiplayerClient)
//            return;
//        Log.Error($"Arenas world bootstrap failed: {reason}");
...
//        Log.Debug($"[ArenaFlow0] Prepare reusable arena requested preset={presetIndex} validPresets={presets.Count} playersReady={playersReady} matchWorldReady={ArenaWorldSystem.MatchReady} netMode={Main.netMode}");
//        if (!playersReady || !validIndex)
//        {
//            Log.Warn($"Arena start rejected: playersReady={playersReady}, validPreset={validIndex}");
//            SetIdle();
//            return;
//        }
...
//        CurrentPresetIndex = presetIndex; Phase = GamePhase.Generating; Result = RoundResult.None; LocalVote = -1;
//        WinningTeam = Team.None; WinningPlayerId = -1; winningAttributionExact = false;
...
//    internal static void PrepareExistingWorldRound(int presetIndex, int nextGenerationId)
//    {
//        if (Main.netMode == NetmodeID.MultiplayerClient || !ArenaWorldSystem.Active || !ArenaWorldSystem.MatchReady)
//            return;
//        List<BossFightPreset> presets = GetValidPresets();
...
//        Log.Chat($"[Arena] Reusing the loaded 4200x1200 world for preset {presetIndex}; no world generation will run.");
//        PrepareSubworldRound(presetIndex, nextGenerationId);
//        MarkSubworldReady(presetIndex, nextGenerationId);
//    }
...
//    {
//        if (candidate == null || player?.active != true || Phase is not (GamePhase.FreezeCountdown or GamePhase.Playing)) return false;
...
//        if (Main.netMode == NetmodeID.Server)
//        {
//            if (selectClass) RoundNetHandler.SendClassState(player.whoAmI, ArenaClass.None, CurrentPresetIndex);
//            else RoundNetHandler.SendApplyKit(player.whoAmI, CurrentPresetIndex);
//        }
//        RoundNetHandler.SendStateToAll();
//        return true;
//    }
...
//        Phase = GamePhase.FreezeCountdown; Result = RoundResult.None; RemainingTicks = countdownTicks >= 0 ? countdownTicks : defaultCountdownTicks; CurrentPresetIndex = presetIndex; LocalVote = -1;
//        WinningTeam = Team.None; WinningPlayerId = -1; winningAttributionExact = false;
...
//            if (Main.netMode == NetmodeID.Server)
//            {
//                if (selectClass) RoundNetHandler.SendClassState(entry.PlayerId, ArenaClass.None, presetIndex);
//                else RoundNetHandler.SendApplyKit(entry.PlayerId, presetIndex);
//            }
//        }
...
//    {
//        if (Phase != GamePhase.FreezeCountdown)
//            return;
//        List<BossFightPreset> presets = GetValidPresets();
...
//    {
//        if (npc == null || Phase != GamePhase.Playing)
//            return false;
//        if (bossIndex >= 0)
...
//    internal static void NotifyBossKilled(NPC npc, Player killer = null, Team reportedTeam = Team.None)
//    {
//        if (Main.netMode == NetmodeID.MultiplayerClient || !IsPrimaryRoundBoss(npc))
//            return;

//        // Linked parts dying is normal for multipart bosses. Only the exact primary
//        // NPC created for this round may complete it.
//        bossDefeatedSignal = true;

//        bool exactAttribution = killer != null || reportedTeam != Team.None;
//        int playerId = -1;
//        Team resolvedTeam = Team.None;

//        if (killer?.active == true && TryGetParticipantTeam(killer.whoAmI, out Team participantTeam))
//        {
//            playerId = killer.whoAmI;
//            resolvedTeam = participantTeam;
//            if (reportedTeam != Team.None && reportedTeam != participantTeam)
//                Log.Warn($"Ignoring mismatched boss winner team {reportedTeam}; participant {killer.whoAmI} belongs to {participantTeam}.");
//        }
//        else if (npc.lastInteraction >= 0 && npc.lastInteraction < Main.maxPlayers
//                 && TryGetParticipantTeam(npc.lastInteraction, out Team interactionTeam))
//        {
//            playerId = npc.lastInteraction;
//            resolvedTeam = interactionTeam;
//            exactAttribution = false;
//        }

//        if (resolvedTeam is Team.Red or Team.Blue
//            && (WinningTeam == Team.None || !winningAttributionExact && exactAttribution))
//        {
//            WinningTeam = resolvedTeam;
//            WinningPlayerId = playerId;
//            winningAttributionExact = exactAttribution;
//        }

//        Log.Info($"Round boss defeated. npc={npc.whoAmI}, type={npc.type}, team={WinningTeam}, player={WinningPlayerId}, exact={winningAttributionExact}.");
//    }
...
//    {
//        if (result != RoundResult.BossDefeated)
//        {
//            WinningTeam = Team.None;
//            WinningPlayerId = -1;
//            winningAttributionExact = false;
//        }
...
//        Log.Debug($"[ArenaFlow9] Ending fight. result={result}, winningTeam={WinningTeam}, winningPlayer={WinningPlayerId}, bossType={bossType}, bossIndex={bossIndex}, active={active}, life={life}, remainingTicks={RemainingTicks}, missingTicks={bossMissingTicks}, spawnAttempts={bossRespawnAttempts}");
//        Result = result;
//        scoreboard.Clear(); scoreboard.AddRange(LiveStats());
//        EndScreenService.Present(EndScreenExtension.CreateSummary(result, WinningTeam, scoreboard));
//        MatchReporter.EndMatch(result, WinningTeam, WinningPlayerId, scoreboard, bossType, CurrentRoundToken);
//        CleanupBoss(); votes.Clear(); ResizeVotes(GetValidPresets().Count);
//        int votingSeconds = Math.Max(1, Config.VotingDurationSeconds);
//        Phase = GamePhase.Voting; RemainingTicks = votingSeconds * 60; LocalVote = -1; IsTimerPaused = false;
//        RoundNetHandler.SendStateToAll();
//    }
...
//        CleanupBoss(); Phase = GamePhase.Idle; Result = result; RemainingTicks = defaultTicks; CurrentPresetIndex = 0; LocalVote = -1;
//        WinningTeam = Team.None; WinningPlayerId = -1; winningAttributionExact = false;
//        CurrentRoundToken = "";
...
//        RoundNetHandler.ResetClientState();
//        Phase = GamePhase.Idle; Result = RoundResult.None; RemainingTicks = DefaultRoundTicks(); CurrentPresetIndex = 0; LocalVote = -1; bossIndex = -1; bossType = 0;
//        WinningTeam = Team.None; WinningPlayerId = -1; winningAttributionExact = false;
//        CurrentRoundToken = "";
//        MatchReporter.Reset();
...
//    private static void SetRemaining(int seconds, int maxSeconds) { RemainingTicks = Math.Clamp(seconds, 0, maxSeconds) * 60; RoundNetHandler.SendStateToAll(); }
