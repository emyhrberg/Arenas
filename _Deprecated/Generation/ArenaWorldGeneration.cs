...
//{
//    // This is intentionally an allow-list. A Plantera world should retain the full
//    // underground/Jungle dependency chain and natural cave dressing, while unrelated
//    // biome branches never execute. Unknown vanilla passes are disabled so a Terraria
//    // update cannot silently add another full biome to the specialized world.
//    private static readonly HashSet<string> PlanteraJunglePasses = new(StringComparer.Ordinal)
//    {
//        // World and terrain foundation.
//        "Reset",
//        "Terrain",
//        ArenaWorldGenerationCatalog.ReserveCombatRegion,
//        "Tunnels",
//        "Dirt Wall Backgrounds",
//        "Rocks In Dirt",
//        "Dirt In Rocks",
...
//        // Underground Jungle formation and its material dependencies.
//        "Jungle",
//        "Mud Caves To Grass",
//        "Dirt To Mud",
//        "Silt",
...
//        // Liquids, walls, structures, and natural underground dressing.
//        "Settle Liquids",
//        "Smooth World",
//        "Waterfalls",
//        "Wall Variety",
...
//        // Arena stamping and final tile normalization.
//        "Tile Cleanup",
//        "Lihzahrd Altars",
//        ArenaWorldGenerationCatalog.BuildCombatRegion,
//        "Final Cleanup",
//        ArenaWorldGenerationCatalog.ValidateCombatRegion
//    };

//    private static readonly string[] RequiredPlanteraJunglePasses =
//    [
//        "Reset",
//        "Terrain",
//        "Jungle",
...
//        ArenaGeneratorKind generator = ResolveActiveGeneratorKind();
//        HashSet<GenPass> disabledByProfile = generator == ArenaGeneratorKind.PlanteraJungle
//            ? ApplyPlanteraJungleProfile(tasks)
//            : [];
...
//        totalWeight = tasks.Where(pass => !disabledByProfile.Contains(pass)).Sum(pass => pass.Weight);
//        string runnable = string.Join(" -> ", tasks.Where(pass => !disabledByProfile.Contains(pass)).Select(pass => pass.Name));
//        Log.Info($"Prepared {tasks.Count - tasks.Count(disabledByProfile.Contains)} runnable deterministic pass(es) for {generator}: {runnable}");
//    }

//    private static ArenaGeneratorKind ResolveActiveGeneratorKind()
//    {
//        BossFightPreset preset = ArenaRoundSystem.GetPresetOrDefault(ArenaSubworldCoordinator.ActiveRequest.PresetIndex);
//        return ResolveGeneratorKind(preset);
//    }

//    private static HashSet<GenPass> ApplyPlanteraJungleProfile(List<GenPass> tasks)
//    {
//        string[] registeredNames = tasks.Select(pass => pass.Name).ToArray();
//        string[] missing = RequiredPlanteraJunglePasses
//            .Where(required => !registeredNames.Contains(required, StringComparer.Ordinal))
//            .ToArray();
//        if (missing.Length > 0)
//            throw new InvalidOperationException($"Cannot build the Plantera Jungle generation profile because required pass(es) are missing: {string.Join(", ", missing)}");

//        HashSet<GenPass> disabled = [];
//        List<string> disabledNames = [];
//        foreach (GenPass pass in tasks)
//        {
//            if (PlanteraJunglePasses.Contains(pass.Name))
//                continue;

//            pass.Disable();
//            disabled.Add(pass);
//            disabledNames.Add(pass.Name);
//        }

//        Log.Info($"Plantera Jungle profile disabled {disabled.Count} unrelated pass(es): {string.Join(", ", disabledNames)}");
//        return disabled;
//    }
...
//            : Main.maxTilesX / 2;
//        if (count == 0)
//            throw new InvalidOperationException($"The Plantera generation profile completed without producing any sampled underground Jungle tiles. jungleOrigin={fallbackX}, scanY={scanTop}..{scanBottom}.");
