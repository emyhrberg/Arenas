...
internal sealed class WorldGenManagerUISystem : ModSystem
{
    ...
    {
        ui = new(); state = new(); state.Append(new WorldGenManagerPanel()); ui.SetState(null);
    }
    ...
    public override void ModifyInterfaceLayers(List<GameInterfaceLayer> layers)
    {
        ...
        if (index < 0) return;
        layers.Insert(index, new LegacyGameInterfaceLayer("Arenas: World Gen Manager", () => { if (IsActive) ui.Draw(Main.spriteBatch, Main._drawInterfaceGameTime); return true; }, InterfaceScaleType.UI));
    }
}
